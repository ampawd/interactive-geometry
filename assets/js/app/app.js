...
(function($, THREE, Global) {
    ...
    function initParams() {
        ...
        cnvParams.renderer = window.WebGLRenderingContext ? new THREE.WebGLRenderer({
            antialias: true
        ...
        cnvParams.cnv.css({
            "left": cnvParams.cnvOffsetX,
            "top": cnvParams.cnvOffsetY
        });
        uiParams.topToolsContainer.css({
            "width": cnvParams.w,
            "margin-left": cnvParams.cnvOffsetX
        });
        cameraParams = {
            distance: 1,
    ...
    function setUP3DView(cnv3DWidth) {
        ...
        $(cnvParams.renderer.domElement).css({
            "margin-left": cnvParams.w + cnvParams.cnvOffsetX + 5,
            "margin-top": 1
        });

        cnvParams.cnv2DOverlay.attr("width", cnv3DWidth);
        cnvParams.cnv2DOverlay.attr("height", cnvParams.h);
        cnvParams.cnv2DOverlay.css({
            "left": parseFloat($(cnvParams.renderer.domElement).css("margin-left")),
            "top": uiParams.topToolsContainerFullHeight
        });
        ...
        let xzPlaneMat = new THREE.MeshLambertMaterial({
            color: 0x11ffff,
    ...
    function updateUI(toolName) {
        ...
        uiParams.subTools.css({
            "visibility": "hidden"
        });
        $("." + toolName).addClass("selected");
        $("." + toolName).find(".sub-tools").css({
            "visibility": "visible"
        });
    }
